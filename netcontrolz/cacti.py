...
class Stem:
    """
    An instance of :py:class:`Stem` represents a single stem (with reference to any associated buds) in a cacti structure.
    """
    ...
    def __contains__(self,x):
        """
        Returns ``True`` if ``x`` is a node in the stem (containment doesn't extend to buds).
        """
        return x in self._node_dict

    def __len__(self):
        """
        Returns the number of nodes in stem (does not include nodes in attached buds).
        """
        return len(self._node_dict)

    def __iter__(self):
        """
        Iterate over the nodes in the stem (not the attached buds).
        """
    ...
    def _extend(self, nodes):
        """
        Extend stem by adding ``nodes`` (``py:iterable``) at the end. Needed when a bud is attached at terminus of the stem, in which case the bud is broken down and stem is extended.
        """
        for u in nodes:
...
class Cycle:
    ...
    def __init__(self, Gi, node_seq):
        """
        Creates a new :py:class:`Cycle` from a sequence of nodes given by ``node_seq``.
        """
        self._node_dict = OrderedDict.fromkeys(node_seq)
        self._G = Gi
        self._droot = None
    ...
    def __contains__(self,x):
        """
        Returns ``True`` if ``x`` is a node in the cycle (containment doesn't extend to buds).
        """
        return x in self._node_dict

    def __len__(self):
        """
        Returns the number of nodes in cycle (does not include nodes in attached buds).
        """
        return len(self._node_dict)
    ...
    def origin(self):
        """
        A starting node (object) of this cycle, in the case of a bud this is the node where
        the distinguished edge is pointing to. In the case of a non-bud cycle, returns None.
        """
        res = self.origin_()
    ...
    def origin_(self):
        """
        A starting node (index) of this cycle, in the case of a bud this is the node where
        the distinguished edge is pointing to. In the case of a non-bud cycle, returns -1.
        """
        if not self.is_bud():
            return  -1
    ...
    def _set_distinguished_root(self,droot,dnode):
        """
        Private method to set the stem or cycle and distinguished node of this cycle when
        this cycle is a bud.
        """
        self._droot = droot
        self._dnode = dnode

    def distinguished_root(self):
        """
        Returns the stem or cycle to which this cycle is attached if it is a bud, otherwise
        return None.
        """
        if not self.is_bud():
            return None
    ...
    def dist_node(self):
        """
        Returns the distinguished node (object) of the stem to which this cycle is attached
    ...
    def dist_node_(self):
        """
        Returns the distinguished node (index) of the stem to which this cycle is attached
    ...
    def dist_edge(self):
        """
        Returns the distinguished edge (u,v) (node objects) where u is in the stem and v is in this cycle/bud of the stem.
    ...
    def dist_edge_(self):
        """
        Returns the distinguished edge e (edge index) where src(e) is in the stem and tgt(e) is in this cycle/bud of the stem.
    ...
    def _set_bud(self, x, bud):
        """
        Private method to add bud to a node x in this cycle.
        """
        self._node_dict[x] = bud
...
def build_cacti_from_matching(G, fixed_ctls, matching, roots=None):
    ...
    **Args**: see method ``build_cacti_from_matching_``
    ...
    fixed_ctls_obj = control_idx(fixed_ctls)
...
def build_cacti_free_controls(G, num_ctls, **kwargs):
    ...
    **Args**:
        * ``num_ctls`` (``int``): Number of control nodes (must be greater than one) that are
          to be attached to the nodes in G. If num_ctls was 0, a degenerate
          matching of only cycles would be found.
...
def build_cacti(G):
    ...
    which also gives the minimum number (and one possible set of locations/configuration) of controls required for the full control of the network.
...
class Cacti:
    ...
    def _build_cacti_from_stemscycles(self):
                    ...
                    for icyc,cyc in enumerate(self._cycles):
                        if cyc and not cyc.is_bud() and v in cyc:
                            ...
                            if u == stem.terminus_():
                                ...
                                stem._set_bud(u, cyc)
                                cyc._set_distinguished_root(stem, u)

        # remove any cycles we extended into a stem
        ...
        nonbud_cycles = [c for c in self._cycles if not c.is_bud()]
        ...
        while len(new_bud_cycles) > 0:
            ...
            for cycle in bud_cycles:
                for u in cycle:
                    for v in self._G.out_neighbors_(u):
                        for icyc,cyc in enumerate(nonbud_cycles):
                            if cyc and not cyc.is_bud() and v in cyc:
                                cyc._reorder_origin(v)
                                cycle._set_bud(u, cyc)
                                cyc._set_distinguished_root(cycle, u)
                                new_bud_cycles.append(cyc)

        # Any remaining non-bud cycles have no distinguished edges from the stems or any existing buds. Now check to see if any of the non-bud cycles have distinguished edges between each other
        nonbud_cycles = [c for c in self._cycles if not c.is_bud()]
        for cycle in nonbud_cycles:
            for u in cycle:
                for v in self._G.out_neighbors_(u):
                    for icyc,cyc in enumerate(nonbud_cycles):
                        if cyc and not cyc.is_bud() and v in cyc:
    ...
    def controls(self):
        """
        Returns list of nodes (objects) where controls are attached.
    ...
    def matching(self):
        """
        Returns the matching used to create this :py:class:`Cacti` object, in form of a list of edges (u,v) where u and v are node objects, as calculated by the maximum matching algorithm (unweighted or weighted as the case maybe). Note that there is degeneracy in the matching, so this may be just one possible matching.
        """
        return [self._G.endpoints(e) for e in self.matching_()]

    def matching_(self):
        """
        Returns the matching used to create this :py:class:`Cacti` object, in form of a list of edge indices, as calculated by the maximum matching algorithm (unweighted or weighted as the case maybe). Note that there is degeneracy in the matching, so this may be just one possible matching.
        """
